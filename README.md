...
**Fase incremental atual: Phase 83 — pan e indicador de mudo master na UI do músico.** A PWA sincroniza pan por canal e envia `SetSendPan`; exibe `MASTER MUTED` como indicador somente leitura. Testes, typecheck e build locais passam. CI remoto executou todos os 9 jobs com sucesso no run `34760297250` após correção do pin de `actions/cache`. Workflow de release agora usa `-rawin` no OpenSSL 3.5. Instalação real, release, hardware e mídia continuam não validados.
