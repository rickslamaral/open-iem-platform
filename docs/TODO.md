...
- **P0-003 media handoff:** `streaming::MediaBridge` bounded non-blocking queue routes `FrameOutput` to per-session media queues; bounded per-session draining is ready for future writer integration; WebRTC session drive and runtime remain pending.
...
- [ ] P0-003 — Media Plane: bounded `MediaBridge` e `SessionRegistry::drive_once` concluídos em CODE/SIMULATED; Opus/RTP media writer, transporte WebRTC e runtime permanecem pendentes.
