...
## Estado atual — 2026-09-17 (P2 Engineer preset validation)
...
- **P2 Preset catalog/application:** `GET /api/v1/presets` permanece catálogo read-only para Musician/Engineer/Admin; `POST /api/v1/presets/{id}/apply` agora aplica presets built-in somente para Engineer/Admin em canal validado. Criação, edição, persistência e aplicação de presets de mix permanecem fora do escopo. Evidência CODE local; runtime permanece pendente.
