...
**Current next step:** Validate clean-environment restore and deployed runtime persistence for SceneStore. Durable export/restore, atomic replacement, active-pointer rollback and Engineer-only authorization are implemented and covered by CODE+CI (PRs #94–#97; latest CI run `35126301166`, 13/13). Do not claim runtime acceptance: PipeWire/ALSA, WebRTC/Opus and Raspberry Pi 5 hardware remain unvalidated. P1-006 release remains blocked by explicit confirmation and physical validation.
