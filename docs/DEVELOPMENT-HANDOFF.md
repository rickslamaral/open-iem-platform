...
**Current next step:** Add bounded transport-output ownership for `str0m::Output::Transmit`; preserve Sans-IO boundaries and test datagram delivery without claiming socket/runtime support. PR #119 merged: the bridge and bounded `SessionRegistry::drive_once` route `FrameOutput`, encode bounded Opus packets and hand them to negotiated Sans-IO `str0m::media::Writer`; polling reports deferred transmit bytes in CODE/SIMULATED only, no network I/O occurs. SceneStore persistence remains covered by fresh `AppState` reconstruction over an explicit SQLite path. Do not claim runtime acceptance: PipeWire/ALSA, WebRTC/Opus and Raspberry Pi 5 hardware remain unvalidated. P1-006 release remains blocked by explicit confirmation and physical validation.
