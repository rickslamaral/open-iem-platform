...
| GAP-016 | P1 | API | START API surface exceeds routes | Missing domain routes | PARTIAL (CODE+CI) | Future ADR | topology/state | GET `/api/v1/system` + GET `/api/v1/channels` implemented; musicians/scenes/presets deferred to P2 per DEVELOPMENT-HANDOFF.md §P2 Queue | Contract tests | No |
...
| GAP-024 | P1 | Operations | Backup/restore library implemented; operational CLI/API and clean restore absent | `config-backup` crate in PR #73; 8 unit tests; CODE+CI/SIMULATED | VALIDATION REQUIRED | Future ADR | 015,020, P1-004 | Add operational backup/restore command and validate clean-environment restore | Clean restore, secret exclusion | No |
...
| GAP-027 | P2 | State | Scenes/state-store durable path and lifecycle | SceneStore SQLite + REST implemented | PARTIAL (CODE+CI) | Future ADR | 015,016 | File-backed path via `SCENE_STORE_PATH`; durable/transient lifecycle and operational backup/restore remain | Reopen persistence, recall/rollback tests | No |
