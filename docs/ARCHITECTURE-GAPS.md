...
| GAP-018 | P1 | Security | Media key/session binding absent; receiver identity registry exists | PairingRegistry integrated into api-server; authenticated offer binds `device_id` to bounded SessionRegistry state, revoke removes active device sessions, and registered SHA-256 DTLS-SRTP fingerprints are matched before session creation | PARTIAL (CODE+CI) | ADR-006 | 001,003,009 | Bind authenticated identity to DTLS-SRTP session | Negative/replay/revoke tests | Yes |
| GAP-019 | P1 | Pairing | Pairing registry exists; runtime reconnect integration absent | PairingRegistry in AppState; pair/revoke/offer-auth integration and fingerprint-binding tests pass in CODE+CI; runtime reconnect and hardware validation remain pending | PARTIAL (CODE+CI) | ADR-006 | 003,018 | Integrate registry with receiver lifecycle | Pair/revoke/reconnect | Yes |
