...
| GAP-018 | P1 | Security | Media key/session binding absent; receiver identity registry exists | PairingRegistry integrated into api-server; authenticated offer now binds `device_id` to bounded SessionRegistry state and revoke removes active device sessions; DTLS-SRTP fingerprint binding remains pending | PARTIAL (CODE+CI) | ADR-006 | 001,003,009 | Bind authenticated identity to DTLS-SRTP session | Negative/replay/revoke tests | Yes |
